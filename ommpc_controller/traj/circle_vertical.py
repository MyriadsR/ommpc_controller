...
for i, time in enumerate(t):
    ...
    # Position (xz plane circle)
    traj[i, 0] = -r * np.cos(angle) + x0
    traj[i, 1] = y0  # y stays constant
    traj[i, 2] = factor * -r * np.sin(angle) + z0
    
    # Velocity (tangential direction in xz plane)
    traj[i, 3] = v_current * np.sin(angle)  # x velocity component
    traj[i, 4] = 0  # y velocity (no motion in y direction)
    traj[i, 5] = factor * -v_current * np.cos(angle)  # z velocity component
...
calculated_v_magnitude = np.sqrt(traj[:, 3]**2 + traj[:, 5]**2)

# write to txt (save in the same directory as this script)
script_dir = os.path.dirname(os.path.abspath(__file__))
output_path = os.path.join(script_dir, 'circle_vertical.txt')
...
# 2. XZ Plane Trajectory (Side View)
ax2 = fig.add_subplot(232)
ax2.plot(traj[:, 0], traj[:, 2], 'b-', linewidth=2, label='Trajectory')
ax2.scatter(traj[0, 0], traj[0, 2], c='r', s=100, label='Start')
ax2.scatter(traj[-1, 0], traj[-1, 2], c='g', s=100, label='End')
ax2.set_xlabel('X (m)')
ax2.set_ylabel('Z (m)')
ax2.set_title('XZ Plane Trajectory (Vertical Circle)')
ax2.axis('equal')
ax2.grid(True)
ax2.legend()
...
ax4.plot(t, traj[:, 5], 'b-', linewidth=2, label='Vz')
...
print("=" * 60)
print(f"Initial Velocity: Vx={traj[0, 3]:.3f}, Vz={traj[0, 5]:.3f}, Total={np.sqrt(traj[0, 3]**2 + traj[0, 5]**2):.3f} m/s")
print(f"Mid Velocity: Vx={traj[num_points//2, 3]:.3f}, Vz={traj[num_points//2, 5]:.3f}, Total={np.sqrt(traj[num_points//2, 3]**2 + traj[num_points//2, 5]**2):.3f} m/s")
print(f"Final Velocity: Vx={traj[-1, 3]:.3f}, Vz={traj[-1, 5]:.3f}, Total={np.sqrt(traj[-1, 3]**2 + traj[-1, 5]**2):.3f} m/s")
